    ...
    ssh -i "$SSH_KEY" "${SSH_USER}@${OCI_MAIN_IP}" << 'MIGRATION_SCRIPT'
        ...
        if [ -f app/config/.env ] || [ -f .env ]; then
            ...
            if command -v psql &> /dev/null; then
                ...
                echo "Running migration 030: Threat Intelligence Tables..."
                PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -U "$DB_USERNAME" -d "$DB_DATABASE" \
                    -f db/migrations/030_threat_intelligence_tables.sql 2>/dev/null || echo "Migration 030 may already be applied"
...
generate_summary() {
    ...
    echo "  - Threat Intelligence APIs (VirusTotal, MalwareBazaar, Hybrid Analysis)"
    echo "  - YARA Scanning & IOC Feeds"
    echo "  - Interactive Malware Sandbox"
    echo "  - CI/CD Integration (GitHub Actions, GitLab CI)"
    echo "  - SBOM Generation & Validation (CycloneDX, SPDX)"
    echo "  - Smart Cryptographic Services (CT monitoring, key expiry alerts)"
    echo "  - Steganography Detection at Scale"
    echo "  - Incident Notifications (Webhooks, Slack, SIEM/SOAR)"
    echo "  - Security Posture Validator Dashboard"
    ...
    echo "  - 030_threat_intelligence_tables.sql (7 new tables)"
    echo ""
